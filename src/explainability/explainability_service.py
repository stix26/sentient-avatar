...
class ModelExplainer:
    ...
    def _explain_with_attention(self, model: nn.Module, input_text: str) -> Dict[str, Any]:
    ...
    def _calculate_token_importance(self, attention_weights: torch.Tensor) -> List[float]:
    ...
    def _setup_routes(self):
        ...
        async def generate_explanation(request: ExplanationRequest):
            try:
                ...
                explanation = self.explainer.explanation_methods[request.explanation_type](
                    model,
                    request.input_text
                )
    ...
    def _log_explanation(
        ...
        try:
            with mlflow.start_run(run_name=f"explanation_{explanation_type}"):
                ...
                # Log metrics
                mlflow.log_metrics({
                    "explanation_time": EXPLANATION_TIME.labels(explanation_type).observe()
                })
