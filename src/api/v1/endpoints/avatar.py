...
def update_cognitive(
    ...
    with COGNITIVE_PROCESSING_TIME.labels(operation=cognitive_in.operation).time():
        avatar = avatar_service.update_cognitive(
            avatar_id, cognitive_in, current_user, db
        )
        if not avatar:
            raise HTTPException(status_code=404, detail="Avatar not found")
        return avatar
...
def update_physical(
    ...
    with PHYSICAL_ACTION_TIME.labels(action=physical_in.action).time():
        avatar = avatar_service.update_physical(
            avatar_id, physical_in, current_user, db
        )
        if not avatar:
            raise HTTPException(status_code=404, detail="Avatar not found")
        return avatar
