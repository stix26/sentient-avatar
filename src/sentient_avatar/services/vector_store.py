...
class VectorStoreService(BaseService):
    ...
    def __init__(self, base_url: str, collection_name: str = "sentient_avatar", timeout: int = 30):
    ...
    async def upsert_points(
        ...
        for i in range(0, len(points), batch_size):
            ...
            try:
                await self._request("PUT", f"/collections/{self.collection_name}/points", json=payload)
            except Exception as e:
                logger.error(f"Error upserting points batch {i//batch_size + 1}: {str(e)}")
                raise
    ...
    async def search_points(
        ...
        try:
            response = await self._request("POST", f"/collections/{self.collection_name}/points/search", json=payload)
            return response
        except Exception as e:
    ...
    async def delete_points(
        self,
        points_selector: Union[List[str], Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            response = await self._request("POST", f"/collections/{self.collection_name}/points/delete", json=payload)
            return response
        except Exception as e:
    ...
    async def get_collection_info(self) -> Dict[str, Any]:
        ...
        """
        try:
            response = await self._request("GET", f"/collections/{self.collection_name}")
            return response
        except Exception as e:
