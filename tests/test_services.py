...
async def test_error_handling(service_factory):
    ...
    # Test rate limiting
    with patch.object(service_factory.rate_limiter, 'is_rate_limited', return_value=True):
        with pytest.raises(Exception):
            await llm.generate("Test")
...
async def test_llm_service_health_check(mock_health_response):
    with patch("aiohttp.ClientSession.get") as mock_get:
        mock_get.return_value.__aenter__.return_value.json.return_value = mock_health_response
        service = LLMService()
