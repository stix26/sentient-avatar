...
async def test_memory_flow(service_factory):
    ...
    # Test vector store
    points = [
        {"id": "1", "vector": [0.1] * 1536, "payload": {"text": "Test memory 1"}},
        {"id": "2", "vector": [0.2] * 1536, "payload": {"text": "Test memory 2"}},
    ]
...
async def test_websocket_flow():
    ...
    async with websockets.connect("ws://localhost:8000/ws/test_client") as websocket:
        # Send text message
        await websocket.send(json.dumps({"type": "text", "text": "Hello"}))
        ...
        await websocket.send(
            json.dumps(
                {"type": "audio", "audio": base64.b64encode(audio_data).decode()}
            )
        )
...
async def test_http_flow():
    """Test HTTP flow."""
    # Test chat endpoint
    response = requests.post("http://localhost:8000/chat", data={"text": "Hello"})
...
async def test_error_handling():
    ...
    response = requests.post(
        "http://localhost:8000/chat", data={}  # Missing required field
    )
    assert response.status_code == 422

    # Test rate limiting
    for _ in range(101):  # Exceed rate limit
        requests.post("http://localhost:8000/chat", data={"text": "Hello"})
