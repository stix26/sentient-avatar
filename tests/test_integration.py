...
async def test_chat_endpoint(client):
    ...
    with patch("sentient_avatar.services.llm.LLMService.generate") as mock_generate:
        with patch(
            "sentient_avatar.services.tts.TTSService.synthesize"
        ) as mock_synthesize:
            with patch(
                "sentient_avatar.services.avatar.AvatarService.generate_video"
                ...
                # Test chat endpoint
                response = client.post("/chat", data={"text": "Hello"})
    ...
    with patch("sentient_avatar.services.asr.ASRService.transcribe") as mock_transcribe:
        ...
        with open("tests/data/test_audio.wav", "rb") as f:
            response = client.post(
                "/transcribe", files={"audio": ("test.wav", f, "audio/wav")}
            )
...
async def test_analyze_image_endpoint(client):
    ...
    # Mock service response
    with patch(
        "sentient_avatar.services.vision.VisionService.analyze_image"
    ) as mock_analyze:
...
async def test_health_endpoint(client):
    ...
    # Mock service responses
    with patch(
        "sentient_avatar.services.llm.LLMService.health_check"
    ) as mock_llm_health:
        with patch(
            "sentient_avatar.services.asr.ASRService.health_check"
        ) as mock_asr_health:
            with patch(
                "sentient_avatar.services.tts.TTSService.health_check"
            ) as mock_tts_health:
                with patch(
                    "sentient_avatar.services.avatar.AvatarService.health_check"
                ) as mock_avatar_health:
                    with patch(
                        "sentient_avatar.services.vision.VisionService.health_check"
                    ) as mock_vision_health:
                        with patch(
                            "sentient_avatar.services.vector_store.VectorStoreService.health_check"
                        ) as mock_vector_store_health:
...
async def test_error_handling(client):
    ...
    # Mock service error
    with patch(
        "sentient_avatar.services.llm.LLMService.generate",
    ...
    response = client.post("/chat", data={})  # Missing required field
